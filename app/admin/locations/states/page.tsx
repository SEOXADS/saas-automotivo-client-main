...
import { MapPin, Plus, Search, Filter, Eye, Edit, Trash2 } from 'lucide-react'
import { locationApiHelpers } from '@/lib/location-api'
import { State, LocationFilters } from '@/types'
...
function StatesPageContent() {
  ...
  const loadStates = async () => {
    setIsLoading(true)
    try {
      const response = await locationApiHelpers.getTenantStates(filters)
  ...
  const handleDelete = async (stateId: number, stateName: string) => {
    const confirmed = confirm(`Tem certeza que deseja excluir o estado "${stateName}"?`)
    if (!confirmed) return

    setIsDeleting(stateId)

    try {
      const success = await locationApiHelpers.deleteTenantState(stateId)
      if (success) {
        alert('Estado excluído com sucesso!')
        loadStates() // Reload the list
      } else {
  ...
  return (
    <AdminLayout title="Estados" subtitle="Gerenciar estados do tenant">
          ...
          <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="Nome ou código do estado..."
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-4">
                {filters.search || filters.is_active !== undefined
                  ...
                  : 'Comece adicionando estados ao seu tenant'
                }
              </p>
              <Button
                onClick={() => router.push('/admin/locations/states/create')}
              ...
              <div className="px-6 py-4 border-b border-gray-200">
                <div className="flex items-center justify-between">
                  <p className="text-sm text-gray-700">
                    Mostrando {pagination.from} a {pagination.to} de {pagination.total} estados
                  </p>
                </div>
              </div>
                    ...
                    {states.map((state) => (
                      <tr key={state.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {new Date(state.created_at).toLocaleDateString('pt-BR')}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <div className="flex items-center justify-end space-x-2">
                            <Button
                              onClick={() => router.push(`/admin/locations/states/${state.id}`)}
                              variant="outline"
                              size="sm"
                            ...
                            >
                              {isDeleting === state.id ? 'Excluindo...' : 'Excluir'}
                            </Button>
                          </div>
                        </td>
                      </tr>
...
export default function StatesPage() {
  return (
    <Suspense fallback={
      <AdminLayout title="Estados" subtitle="Gerencie os estados do sistema">
        <div className="p-6">
