...
export const locationApiHelpers = 
{
  ...
  async createTenantCountry(data: CountryFormData): Promise<Country | null> {
    try {
      ...
      const payload = {
        ...
        phone_code: data.phone_code?.trim() || null,
  ...
  async getPublicStatesByCountry(countryId: number): Promise<State[]> {
    try {
      const response = await adminApi.get(`${ADMIN_API_CONFIG.ENDPOINTS.PUBLIC_STATES}?country_id=${countryId}&per_page=1000`) as { data?: State[] }
      return response.data || []
    } catch (error) {
      console.error('Erro ao buscar estados públicos por país:', error)
      return []
    }
  },
